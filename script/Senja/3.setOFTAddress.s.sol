...
contract SetOFTAddress is Script, Helper {
    function run() public {
        ...
        console.log(
            "oftaddress of kaia_usdt before set: ",
            IFactory(address(KAIA_lendingPoolFactoryProxy)).oftAddress(KAIA_USDT)
        );
        IFactory(address(KAIA_lendingPoolFactoryProxy)).setOftAddress(KAIA_USDT, KAIA_OFT_USDT_ADAPTER);
        console.log(
            "oftaddress of kaia_usdt after set: ", IFactory(address(KAIA_lendingPoolFactoryProxy)).oftAddress(KAIA_USDT)
        );
        IFactory(address(KAIA_lendingPoolFactoryProxy)).setOftAddress(
            KAIA_USDT_STARGATE, KAIA_OFT_USDT_STARGATE_ADAPTER
        );
